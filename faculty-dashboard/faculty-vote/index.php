<?php
    ...
    $get_temp = "SELECT right((verification_code),4) voters_cnt
                ...
                WHERE election_id='$election_id_fce' AND status_vote='Y'
            ...
            <main class="mt-4"> 
                <div class="container col-sm-12">  
                    ...
                    <?php } else if ($status =='In-Progress' && ($my_status_vote == 'N' || $my_status_vote == '')) {?>
                              ...
                              <!-- Vote Section -->
                        <section class="content-wrapper position-relative h-100">
                            <h4 class="title fw-bold text-center mb-4"><?php echo $election_year.' '.$description;?><h4>
                            ...
                            <form method="post" class="needs-validation" novalidate>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                            ...
                                            <?php
                                                    $sql    =  "SELECT a.rec_id, a.student_id, a.first_name, a.middle_name, a.last_name, a.position, a.platform, a. election_id, a.dept_id, a.year, b.position_name, profile_pic
                                            ...
                                            <input type="hidden" id="TRE"  value="">
                                            <input type="hidden" id="AUD"  value="">
                                            <input type="hidden" id="PRO"  value="">
                                            <input type="hidden" id="SGA"  value="">
                                            <input type="hidden" id="BM"  value="">
                                    </div>
                                </div>
...
<script>
    ...
    function verify(x) {
        if ($('#chkbox'+[x]).is(":checked")) {
            ...
            if(positionID =="SGA"){
                ...
                if(sga =="0" || sga ==""){
                    var value = parseInt(document.getElementById('SGA').value, 10);
                    ...
                    Swal.fire('You can only vote 2 '+ position,'', 'error') 
                    $('#chkbox'+[x]).prop('checked', false);  
            ...
            if(positionID =="PD" ){
                ...
                $('#VP').val('');  
            } else if (positionID =="SC") {
                $('#SC').val('');  
            } else if (positionID =="TRE") {
                $('#TRE').val('');  
            } else if (positionID =="AUD") {
                $('#AUD').val('');  
            } else if (positionID =="PRO") {
                $('#PRO').val('');  
            } else if (positionID =="SGA") {
                var value = parseInt(document.getElementById('SGA').value, 10);
                value = isNaN(value) ? 0 : value;
                value--;
                $('#SGA').val(value); 
    ...
    $(document).on("click", "#btnVote", function(event) {
        ...
        if (pd !='' && vp !='' && sc !='' && tre !='' && aud !='' && pro !='' && sga !='' && bm !='') {
            var label_no_vote = "";
        } else { 
        ...
        if (sc =="") {
            var pos_SC = "Secretary"; 
            var pos_SC_br = "<br>";
        } else { var pos_SC = ""; var pos_SC_br = "";}

        if (tre =="") {
            var pos_TRE = "Treasury";  
            var pos_TRE_br = "<br>";
        } else { var pos_TRE = ""; var pos_TRE_br = "";}

        if (aud =="") {
            var pos_AUD = "Auditor"; 
            var pos_AUD_br = "<br>";
        } else { var pos_AUD = ""; var pos_AUD_br = "";}

        if (pro =="") {
            var pos_PRO = "PRO";
            var pos_PRO_br = "<br>";
        } else { var pos_PRO = "";  var pos_PRO_br = "";}

        if (sga =="") {
            var pos_SGA = "Sergeant and Arms"; 
            var pos_SGA_br = "<br>";
        } else { var pos_SGA = ""; var pos_SGA_br = "";}

        if (bm =="") {
            var pos_BM = "Business Manager";
            var pos_BM_br = "<br>";
        } else { var pos_BM = ""; var pos_BM_br = "";}
        ...
        // }else {  
            ...
            Swal.fire({
                ...
                html: label_no_vote+"<h5 class='text-red mt-n4'>"+pos_PD+pos_PD_br+pos_VP+pos_VP_br+pos_SC+pos_SC_br+pos_TRE+pos_TRE_br+pos_AUD+pos_AUD_br+pos_PRO+pos_PRO_br+pos_SGA+pos_SGA_br+pos_BM+pos_BM_br+"</h5><br><h4>Are you sure you want to submit your VOTE?</h4>",
                type: 'warning',
