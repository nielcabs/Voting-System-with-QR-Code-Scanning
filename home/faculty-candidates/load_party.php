       ...
       // $query = "WHERE election_id = '$election_id' AND election_status = 'N'" ?? '';
        ...
        if ($cnt > 0) {
            $query = "WHERE election_id = '$election_id' AND party_name IN (SELECT party_name 
                                                                            ...
                                                                            SELECT party_name
                                                                            FROM tbl_party
                                                                            WHERE election_id = '$election_id' AND election_status = 'N' 
                                                                            AND party_name NOT IN (SELECT party_name 
                                                                                                   FROM tbl_candidates
        ...
        if ($cnt > 0) {
            $query = "WHERE election_id = '$election_id' AND party_name NOT IN (SELECT party_name 
                                                                                FROM tbl_candidates
                                                                                WHERE election_id = '$election_id' AND position='$position'
                                                                                AND party_name IN ('$cand_pt_name'))" ?? '';
        } else {
            $query = "WHERE election_id = '$election_id'" ?? '';
        }
    }
